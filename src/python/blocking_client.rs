...
impl PyBlockingClient {
    ...
    fn connect(py: Python<'_>, addr: String, config: &PyRpcConfig) -> PyResult<Self> {
        ...
        let client = py.allow_threads(|| {
            runtime.block_on(async {
                RpcClient::connect(socket_addr, rust_config).await
            })
    ...
    fn call_batch(&self, py: Python<'_>, requests: Vec<(String, Vec<u8>)>) -> PyResult<Vec<PyObject>> {
        let client = self.inner.clone();
        let runtime = get_runtime();
