...
**💡 Quick Tip**: You can use the helper scripts `./run_director.sh` and `./run_worker.sh` from this directory to run the Rust components without changing directories. See the detailed examples below.

### Terminal 1: Start the Director

The director coordinates the cluster and routes requests to workers.

**Option A - Using the helper script (from this directory):**
```bash
cd examples/python/cluster
./run_director.sh
```

**Option B - Traditional approach (from examples/cluster):**
```bash
cd examples/cluster
...
**Option A - Using the helper script (from examples/python/cluster):**
```bash
cd examples/python/cluster
./run_client.sh
```
